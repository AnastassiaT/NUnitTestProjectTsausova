...
namespace NUnitTestProjectTsausova
{
    public class UnitTest1
    {
        ...
        public void test_page1()
        {
            ...
            for (int a = 0; a < 10; a++)
            {
                ...
                {
                    try { sText[i].Click(); sText[i].SendKeys("Tsausova"); } catch (Exception) { }
                }
                var sTextArea = driver.FindElements(By.XPath("//textarea"));
                for (int i = 0; i < sTextArea.Count; i++)
                {
                    try { sTextArea[i].Click(); sTextArea[i].SendKeys("TsausovaAnastassia"); } catch (Exception) { }
                }
